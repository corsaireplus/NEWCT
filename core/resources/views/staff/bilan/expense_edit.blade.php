...
@section('panel')
    <div class="row mb-none-30">
        <div class="col-lg-12 mb-30">
            <div class="card">
                <div class="card-body">
                    <form action="{{route('staff.transaction.update_depense')}}" method="POST">
                        @csrf
                        <input type="hidden" id="id" name="id" value="{{$depense->id}}">
                        <div class="row">
                            <div class="form-group col-lg-6">
                                ...
                                <select class="form-control form-control-lg" id="cat_id" name="cat_id">
                                    @foreach($categorie as $cat)
                                        <option value="{{ $cat->id }}" @if(old($depense->cat_id) === $cat->id) 'selected' @endif >{{__($cat->nom)}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group col-lg-6">
                        ...
                        <div class="row">
                            <div class="form-group col-lg-6">
                                ...
                                <input type="date" class="form-control form-control-lg" id="date" name="date"   maxlength="40" required="" value="{{ $depense->date }}">
                            </div>

                             <div class="form-group col-lg-6">
                                <label for="lname" class="form-control-label font-weight-bold">@lang('Montant')</label>
                                <input type="numeric" class="form-control form-control-lg" id="montant" name="montant"   maxlength="40" value="{{ $depense->montant }}" required="">
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="form-group col-lg-12">
                                ...
                                <textarea name="description" id="description" rows="6" class="form-control form-control-lg" placeholder="@lang('Observation ou Note')">{{$depense->description}}</textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <button type="submit" class="btn btn--primary btn-block"><i class="fa fa-fw fa-paper-plane"></i> @lang('Modifier Depense')</button>
                        </div>
                    </form>
                </div>
            </div>
